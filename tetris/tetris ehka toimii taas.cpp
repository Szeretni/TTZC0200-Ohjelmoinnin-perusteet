...
HIGHSCORE huippupisteet[10] = { NULL };
...
void TallennaPisteet(int pisteet, HIGHSCORE huippupisteet[10])
{
	...
	cout << "Anna nimesi: ";
	cin >> huippupisteet[1].nimi;
	huippupisteet[1].score = pisteet;
	string paiva = __DATE__;
	huippupisteet[1].paiva = paiva;
	for (int i = 0; i < 1; i++)
	{
		kirjoitus << "\"NIMI\"" << ":" << huippupisteet[1].nimi << endl;
		kirjoitus << "\"PISTEET\"" << ":" << huippupisteet[1].score;
		kirjoitus << "\"PAIVAMAARA\"" << ":" << huippupisteet[1].paiva;
	}
	kirjoitus.close();
}
...
int Valikko()
{
	cout << "Tetris by Hannu Oksman (c)2017\n";
	cout << "\nValikko";
	cout << "\n1) Pelaa";
	cout << "\n2) Huippupisteet";
	cout << "\n3) Lopeta\n";

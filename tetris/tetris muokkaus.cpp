...
int main()
{
	...
	for (int i = 0; ; i++)
	{
		...
		for (rivi = 0; rivi < dimensio; rivi++)
		{
			...
			{
				char apu_tarkistuspointteri = *tarkistuspointteri;
				int apu_j = j;
				char *poistopointteri = NULL;
				if ((*tarkistuspointteri) == 'x')
				{	
					/*ALKAA TARKISTUS ONKO SEURAAVA MERKKI SAMAA*/
					...
					if (montakosamaamerkkia >= perakkainoltava)
					{
						...
						*tarkistuspointteri = apu_tarkistuspointteri;
						j = apu_j;
					}
					else
					{
